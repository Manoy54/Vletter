...
function createHeartGeometry() {
    ...
    const s = 0.05;
    shape.moveTo(0, s * 5);
    shape.bezierCurveTo(0, s * 5, -s * 1, s * 9, -s * 5, s * 9);
    shape.bezierCurveTo(-s * 10, s * 9, -s * 10, s * 3.5, -s * 10, s * 3.5);
    shape.bezierCurveTo(-s * 10, 0, -s * 7, -s * 3.5, -s * 3.5, -s * 6);
    shape.bezierCurveTo(-s * 1, -s * 8, 0, -s * 11, 0, -s * 12);
    shape.bezierCurveTo(0, -s * 11, s * 1, -s * 8, s * 3.5, -s * 6);
    shape.bezierCurveTo(s * 7, -s * 3.5, s * 10, 0, s * 10, s * 3.5);
    shape.bezierCurveTo(s * 10, s * 3.5, s * 10, s * 9, s * 5, s * 9);
    shape.bezierCurveTo(s * 1, s * 9, 0, s * 5, 0, s * 5);
...
class Z extends InstancedMesh {
    ...
    constructor(renderer: WebGLRenderer, params: Partial<typeof DefaultConfig> = {}) {
        ...
        // Random initial rotation (x, y, z) per instance
        this.rotationData = new Float32Array(3 * config.count);
        // Random spin speed per instance
        this.spinData = new Float32Array(3 * config.count);
        for (let i = 0; i < config.count; i++) {
            const b = 3 * i;
            this.rotationData[b] = Math.random() * Math.PI * 2;
            this.rotationData[b + 1] = Math.random() * Math.PI * 2;
            this.rotationData[b + 2] = Math.random() * Math.PI * 2;
            this.spinData[b] = (Math.random() - 0.5) * 0.8;
            this.spinData[b + 1] = (Math.random() - 0.5) * 0.8;
            this.spinData[b + 2] = (Math.random() - 0.5) * 0.4;
        }
        this.#setupLights();
    ...
    update(deltaInfo: { delta: number }) {
        ...
        for (let idx = 0; idx < this.count; idx++) {
            const b = 3 * idx;
            // Update rotation with gentle spin
            this.rotationData[b] += this.spinData[b] * deltaInfo.delta;
            this.rotationData[b + 1] += this.spinData[b + 1] * deltaInfo.delta;
            this.rotationData[b + 2] += this.spinData[b + 2] * deltaInfo.delta;

            DummyObj.position.fromArray(this.physics.positionData, b);
            DummyObj.rotation.set(
                this.rotationData[b],
                this.rotationData[b + 1],
                this.rotationData[b + 2]
            );
